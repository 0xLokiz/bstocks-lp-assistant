...
其中 `σ` 是代币的年化波动率，用链上K线历史估算（对数收益率标准差，按
`sqrt(365)` 年化）。
...
关——这比单纯看APY排名、甚至比单看 `net_apy` 数字，更能做到跨池子（APY/波
动率量级差异巨大时）的苹果对苹果比较。
...
- **跨价区间的"从未离开"概率是个宽松的保守下界**——对窄区间来说，即便真实
  概率是个不大的正数，它也可能显示 `0%`。这是并集上界近似本身宽松，不是说
  真的不可能。
...
*（图表：横轴 p_active，纵轴 net_apy，每个候选区间一个点，推荐的±50%区间
...
- **分时段波动率。** Fables 的"Calendar"费率模型之所以在交易时段/盘后/周
  ...
  因。
- **精确的双边界概率**，用严谨的反射原理级数解替换现在跨价区间保守的并集
  上界近似（`no_exit_probability`）——收紧窄区间现在容易读出的 `0%`，同时
  不改变安全方向的偏差。
- **单边区间的真实成交价模型**——现在 `--side sell`/`buy` 区间还在借用
  IL-vs持有公式当成本代理；真正的问题（"相比边界处的普通限价单，我实际的
  平均成交价是多少"）需要自己的模型，不是借来的。
- **波动率不确定性感知评分。** `vol_ratio` 目前用的是已实现波动率的点估
  计；K线历史短的话这个估计会很吵。在计算 `vol_ratio` 之前用置信区间放宽
  `σ`（或者换成GARCH类估计器），能避免薄弱的数据窗口显得过于精确。
