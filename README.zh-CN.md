...
> 护的——它是会犯错的。就在一次真实使用的会话里，先后发现并修好了两个
> 真实的计算bug（见下面"最近完成的"）——两次都是先给出一个看起来很确
> 定、实际上是错的具体数字，然后才被抓出来。每个命令的输出末尾都会带
> 同样的提醒。请把这里的每一个数字都当成一个需要自己独立验证的模型估
> 计，绝不要当成可以直接照做的事实——尤其是在真的要存入资金之前。
...
**N/A**，而不是一个数字（早先的版本在这里封顶到100%，看起来挺安全，
但近似公式已经离开了它本该适用的范围，封顶数字照样是假精度——这是拿
一个真实存在、波动率就是这么大的池子验证过的，不是编出来的极端情况，
见"最近完成的"）。集中区间的数字没有这个问题——它们锚定的是精确的边
界IL，而不是这个近似公式，所以在任何波动率下都有效。
...
是在被要求去核查`cli.py`之后自己直接查代码发现的，不是反过来——另一个独立
运行这个skill的Claude Code会话，报告说GME的收益率是"5.96%"、第二名是
"3.08%"，这两个数字跟这边算出来的任何东西都对不上。追查下来：`cmd_recommend`
的标题那一行用的是`best_range.model_net_apy`（集中区间的数字，比如某个
实测的GMEB-USDT池子当时是845.3%），而紧跟在下面的排名表格、以及再下面的
`$/yr`资金估算，用的都是同一个池子的满区间`model_net_apy`（同一时刻是
593.97%）——两个数字都算对了，但输出里没有任何地方说明这是两个不同的指
标，也没说明它们为什么会不一样，读的人（不管是人还是另一个agent）根本没
法把这个跟bug或者矛盾区分开。（那个会话报的"5.96%"/"3.08%"，几乎正好是
满区间数字又除了一次100——是那个会话自己独立代码里的单位问题，不是这个
工具本身算错了，但这个工具输出里本来就存在的这种歧义，是造成误读的根源。）
修复方式是让 `cmd_recommend` 和 `scan --with-range --capital` 里的表格和
资金估算，都改成用每一行自己的 `best_range.model_net_apy`——跟标题那行引
用的是同一个数字——而不是悄悄退回到满区间的数字。新增了两个回归测试，锁定
标题、表格、资金估算三处对同一个池子的数字必须一致。
...
用户直接要求把 Fables 的文档（fables.fi/docs）完整读一遍，找有什么值得
借鉴的地方，直接改。读了概览、swap-fee、risks、security、methodology、
permissions、addresses、fees、providing-liquidity、price-moves、APR这些
页面。发现这个项目已有的两个设计决策，正好跟一个真实运行中的V4 hook协议
自己文档里写的东西对得上，所以真正有价值的改动是把这层佐证精确地引用进
来，而不是凭空发明新机制：(1) Fables 自己的数据方法论页面证实了它也是用
"24小时手续费窗口除以当前TVL"来给池子头部APR年化——正是这个项目
`MODEL_APY_CAVEAT` 已经从自己的真实观测（同一个池子的TVL/apy在几分钟内
剧烈摆动）里警告过的那个机制——它的APR页面还明确说这个数字不是预测，也
没有扣掉相对于持有的价值差异，这正是为什么 `model_net_apy` 要从平台原始
`apy` 里减掉估算的IL，而不是直接把原始数字端出来；(2) Fables 的安全页面
写明目前没有发布任何针对Fables本身的审计报告——这是实锤而不是假设，直接
证明"是一个真实存在的V4 hook协议"从来不能证明某个具体hook是安全的，进
一步支撑了这个工具对V4池子默认硬拦截的设计。把具体、可核实的引用（方法
论/APR/安全页面的地址、三种命名费率模型、keeper覆盖的具体边界：最多5折、
最长72小时、写死在字节码里的绝对上限）加进了 `MODEL_APY_CAVEAT`、
`MODEL.md` 第7/9节，以及两份README的V4硬拦截和注意事项部分。另外单独核
查了这个项目自己代码里 `pool.get("apy") or 0` 这一类兜底写法，看有没有
重复Fables方法论页面明确警告过的"把缺失数据当成确定的零"这个错误——结果
没有：每一处要么是一条已经有优先级排序的多来源兜底链里的最后一层，要么
失败方向是把池子排除/标记为不吸引人，而不是错误地读成"安全"或"有吸引
力"——是安全的失败方向，所以这里不需要改代码。同时用文档里现在能直接确
认的信息（chain ID 4663、三种费率模型的名字），把Roadmap里Robinhood
Chain和分时段波动率这两项从"推断"改成了"引用"。

### 最近完成的（加了一条常驻的DYOR/风险提示，不只是一条caveat）

用户直接提出的要求，就在同一个会话里刚发现并修好两个真实计算bug之
后：每一个给出风险/收益判断的命令，都需要每次提醒用户——这个工具的输
出来自AI辅助、还在持续开发中的代码给出的模型估计，不是已验证的事
实——这一点跟某个具体数字的caveat无关，是独立存在的。新增了
`config.DYOR_DISCLAIMER`，在 `scan`/`range`/`recommend`/
`rebalance-check` 的文本输出末尾打印，也作为 `dyor_disclaimer` 字段带
在每一个有 `--json` 输出的命令里（`cmd_range` 之前压根没打印过任何
caveat——这是顺带发现并补上的一个真实、独立的缺口）。`SKILL.md` 现在
明确说这条提示是常驻要求，每次展示评级/区间/结论的时候都要带上，不是
跟现有"只给一条最相关的caveat，别列一堆"规则抢位置的又一条caveat——
两者回答的是不同的问题，都重要。两份README和INSTALL.md现在开头都是同
样的免责声明，直接拿这两个真实的bug当例子，而不是一句空泛的警告。四
个命令都新增了测试，确认提示真的出现在实际输出里。
...
下面那个"封顶到100%"的修复（同一天）结果发现并不是正确的修法——被同
一个真实用户、同一个会话里抓到了："但实际上IL不会这样，碰到这种你应
该给出NA"。说得对。`σ = √8 ≈ 283%` 年化以上，`expected_il_fraction`
用的扩散近似公式已经不只是撞到了天花板——它已经离开了自己本该适用的
小`σ`范围，所以封顶到1.0照样是假精度，只不过从一个明显荒谬的数字换成
了一个"看起来安全"的数字而已。`expected_il_fraction` 现在在这个门槛之
后返回 `None`；`risk_adjusted_apy`、`recommend_range` 里合成的满区间那
一行、以及 `run_scan`，全都老老实实地把这个 `None` 传递下去——波动率
这么大的池子，满区间这个数字既没法排名也没法给出结论，所以现在会归入
`unscoreable`（跟其他"sigma/apy都解出来了、但还是评不了分"的情况用同
一个桶），而不是悄悄给一个建立在不存在的数字上的 WATCH/ENTER 结论。
`cmd_range` 的文本表格里对应那一行打印 `N/A`，还带一行解释；区间推荐
本身也绝不会选中这种 `None` 行——会自动退回到某个集中区间，因为那些数
字锚定的是精确边界IL而不是这个扩散近似，在任何波动率下都有效。新
增/改了5个测试，MODEL.md和两份README的公式又改正了一遍。还是拿引出这
两轮问题的同一个真实池子（DJTB）验证过的。

### 最近完成的（MODEL_APY_CAVEAT：也是交易量/TVL噪音，不只是激励）

紧接着下面那个IL封顶修复，来自同一个问题（"这个池子的apy为什么跟之前
差这么多"）：直接查了一下真实的 `defi investment-info` 响应——里面根
本没有交易量字段，也没有任何时间戳，只有一个预先混合好的
`apy`/`apyBps`。这意味着这个工具根本没办法分辨"这是稳定的手续费收
益"还是"刚好在这次快照之前有一笔大额交易或短暂的成交量高峰把数字冲上
去了"——这是跟"激励项目结束"完全不同的另一种风险，之前的caveat文案没
提到。`MODEL_APY_CAVEAT` 和 MODEL.md 的局限性清单现在都明确说了，还
拿引出这个问题的真实数字举了例子（同一个池子，两次相隔几分钟的查询之
间TVL几乎腰斩、apy涨了约三倍）。
