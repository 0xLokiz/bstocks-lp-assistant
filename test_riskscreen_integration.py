...
class FakeGet:
    ...
    def __call__(self, url, params):
        if url == riskscreen.RWA_LIST_URL:
            return {"success": True, "data": self.stock_tokens}
        if url == riskscreen.KLINE_URL:
...
def _clear_memoized_fetch_caches():
    ...
    # for a single real CLI process -- see riskscreen.py). Across tests in one pytest process
    # that cache would leak fixture data between tests, so clear it before and after every test.
    riskscreen.fetch_stock_tokens.cache_clear()
    riskscreen.fetch_klines.cache_clear()
    yield
    riskscreen.fetch_stock_tokens.cache_clear()
    riskscreen.fetch_klines.cache_clear()
...
def fake_io(monkeypatch):
    ...
    monkeypatch.setattr(riskscreen, "baw", fb)
    monkeypatch.setattr(riskscreen, "_get", fg)
    return fb, fg
...
def test_fetch_stock_tokens_raises_clear_error_on_missing_success_field(fake_io, monkeypatch):
    fb, fg = fake_io
    monkeypatch.setattr(riskscreen, "_get", lambda url, params: {"data": []})  # no "success" key
    with pytest.raises(RuntimeError, match="RWA list fetch failed"):
        riskscreen.fetch_stock_tokens()


def test_fetch_klines_raises_clear_error_on_failed_body(fake_io, monkeypatch):
    monkeypatch.setattr(riskscreen, "_get", lambda url, params: {"success": False, "error": "boom"})
    with pytest.raises(RuntimeError, match="kline fetch failed"):
        riskscreen.fetch_klines("56", "0xabc")
...
def test_fetch_investment_info_raises_on_baw_error_envelope(fake_io):
    ...
    with pytest.raises(RuntimeError):
        riskscreen.fetch_investment_info("does-not-exist")
...
def test_baw_raises_clear_error_on_nonzero_exit_no_json(monkeypatch):
    ...
    monkeypatch.setattr(riskscreen, "_resolve_baw_path", lambda: "C:\\fake\\baw.CMD")
    monkeypatch.setattr(riskscreen.subprocess, "run", lambda *a, **k: FakeCompletedProcess())

    with pytest.raises(RuntimeError, match="not signed in"):
        riskscreen.baw("defi", "position")


def test_baw_rejects_shell_metacharacters_before_touching_subprocess(monkeypatch):
    calls = []
    monkeypatch.setattr(riskscreen, "_resolve_baw_path", lambda: "C:\\fake\\baw.CMD")
    monkeypatch.setattr(riskscreen.subprocess, "run", lambda *a, **k: calls.append(1))

    with pytest.raises(ValueError):
        riskscreen.baw("defi", "investment-info", "--investmentId", "123 & calc.exe")
...
def test_scan_json_is_pure_json_with_envelope(fake_io, capsys):
    ...
    args = riskscreen.build_parser().parse_args(["scan", "--top", "5", "--json"])
    riskscreen.cmd_scan(args)

    out = capsys.readouterr().out
    data = json.loads(out)  # raises if anything but pure JSON was on stdout
    assert data["schema_version"] == riskscreen.SCHEMA_VERSION
...
def test_positions_json_is_pure_json_with_envelope(fake_io, capsys):
    ...
    args = riskscreen.build_parser().parse_args(["positions", "--json"])
    riskscreen.cmd_positions(args)
...
def test_rebalance_check_json_error_envelope_on_position_fetch_failure(monkeypatch, capsys):
    ...
    monkeypatch.setattr(riskscreen, "fetch_positions", boom)

    args = riskscreen.build_parser().parse_args(["rebalance-check", "--json"])
    with pytest.raises(SystemExit):
        riskscreen.cmd_rebalance_check(args)
...
def test_rebalance_check_evaluates_every_investment_id_worst_case(monkeypatch, capsys):
    ...
    monkeypatch.setattr(riskscreen, "fetch_positions", fake_fetch_positions)
    monkeypatch.setattr(riskscreen, "fetch_stock_tokens", lambda type_filter=None: [nvda_token()])
    ...
    monkeypatch.setattr(riskscreen, "fetch_investment_info", fake_fetch_investment_info)
...
def test_no_exit_probability_stays_in_unit_interval_property():
    ...
    for _ in range(200):
        ...
        result = riskscreen.no_exit_probability(pa, pb, sigma, years)
...
def test_switching_recommendation_verdict_matches_gap_sign_property():
    ...
    for _ in range(200):
        ...
        result = riskscreen._switching_recommendation(position_usd, held, alt)
